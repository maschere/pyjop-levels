...
class DataModel(DataModelBase):
    ...
    def spawn_next(self):
        ...
        for i in range(0, 16):
            ...
            x = (i % 4 - 1.5) / 1.5  * 1.2
            y = (i //4 - 1.5) / 1.5  * 1.2
            editor.spawn_static_mesh(SpawnableMeshes(cname), simulate_physics=True, is_temp=True, location = (5.000 + x + random.uniform(-0.15,0.15),0.150 + y + random.uniform(-0.15,0.15),0.3 + random.uniform(0,.2)), rotation = (0,0, random.uniform(-180,180)), scale=2)
...
editor.spawn_entity(SpawnableEntities.SmartCamera, "reference_cam", location=(-5.15, 0.15, 3.75), rotation=(0,-90,0))
editor.spawn_static_mesh(SpawnableMeshes.Cube, scale= (3.5,3.5,0.2), location=(-5, 0.15, 0), material=SpawnableMaterials.SimpleColor, color=Colors.Purple)
i = 0
...
editor.set_goals_intro_text(f"Let's improve our coin counting machine. Use the camera to sum up the value of all coins on the green slab. Enter the result in the InputBox. Do this {NUM_RUNS} times.")
...
editor.add_hint(0,["What is the other camera for?","What are the coins on the purple slab for?", "How can I get reference images for all coins?"], "All Euro coins are available for reference on the purple slab. Use the camera called 'reference_cam' to get an image of them.")
### END HINTS ###
...
def on_input(box:InputBox,gt:float,txt:str):
    ...
    if box.entity_name == "CoinCount" and data.count == val:
        ...
        prog = data.num_sum_correct / NUM_RUNS
        editor.set_goal_progress(box.entity_name, prog)
        if prog < 1:
            data.spawn_next()
        return
