...
class DataModel(DataModelBase):
    def __init__(self) -> None:
        ...
        self.incorrect_delivery = False
        self.incorrect_discard = False
...
editor.spawn_entity(SpawnableEntities.ConveyorBelt, "belt2", location=(3.45, -4.0, 0.0), scale=(1.000,1.000,0.842), is_controllable=False) #generated-code

editor.spawn_entity(SpawnableEntities.TurnableConveyorBelt, "turnable_belt1", location=(-1.55, -4.0, 0.0), is_controllable=False)
...
editor.spawn_entity(SpawnableEntities.ObjectSpawner, unique_name="spawner1", location=(-8.45, -3.95, 3.7), rotation=(0.0, 0.0, -90.0), is_controllable=False)
...
editor.spawn_entity(SpawnableEntities.DeliveryContainer, "container1", location=(7.05, -4.0, 0.0)) #generated-code
...
def spawn_next():
    ...
    # box quality
    quality = random.choices(["ok", "defected"], weights=[55, 45], k=1)[0]
...
editor.set_goals_intro_text(f"Deliver 5 good quality products and discard bad ones. Use the quality scanner to analyze the boxes and use the buttons to control the delivery line.")
...
def level_goal(goal_name: str):
    ...
    editor.set_goal_progress(
        goal_name,
        data.correct / 5,
        f"Products delivered: {data.correct}/5"
    )

editor.specify_goal("deliver_products", f"Products delivered: {data.correct}/5.", level_goal)
...
def on_toggle_handler(sender,simtime,is_on): 
    if is_on:
        ...
        TurnableConveyorBelt.first().set_target_speed(3)  # Move when OFF
...
def on_reset():
    ...
    ConveyorBelt.find("belt1").set_target_speed(3)
    ConveyorBelt.find("belt2").set_target_speed(3)
    
    if ToggleSwitch.first().get_is_switched_on() == False:
        TurnableConveyorBelt.first().set_target_speed(3)
