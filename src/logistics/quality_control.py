...
editor.spawn_entity(SpawnableEntities.ConveyorBelt, "belt2", location=(3.600,-4.000,0.000), scale=(1.000,1.000,0.842)) #generated-code

editor.spawn_entity(SpawnableEntities.TurnableConveyorBelt, "turnable_belt1", location=(-1.45, -4.0, 0.0), is_controllable=False)

editor.spawn_entity(SpawnableEntities.PushButton, "button1", location=(-4.25, -2.1, 2.45), rotation=(90.0, 0.0, 0.0))
editor.spawn_entity(SpawnableEntities.PushButton, "button2", location=(-4.25, -2.1, 1.3), rotation=(90.0, 0.0, 0.0))
...
editor.spawn_entity(SpawnableEntities.ToggleSwitch, unique_name="toggle1", location=(-4.15, -2.1, 1.95), rotation=(-90.0, 0.0, 180.0))
...
editor.spawn_entity(SpawnableEntities.MovablePlatform, "platform1", location=(-1.40,-1.20,0.5)) 
...
def on_reset():
    ...
    if ToggleSwitch.first().get_is_switched_on(): ToggleSwitch.first().toggle()
    data.reset()
...
def on_player_command(gametime:float, entity_type:str, entity_name:str, command:str, val:NPArray):
    ...
    if entity_name == "button1" and command == "Press":
    ...
    if entity_name == "button2" and command == "Press":
...
def on_tick(simtime: float, deltatime: float):
    spawn_next()
    if ToggleSwitch.first().get_is_switched_on():
        TurnableConveyorBelt.first().set_target_speed(4)
    else: TurnableConveyorBelt.first().set_target_speed(0)
